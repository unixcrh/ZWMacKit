...
@interface ZWOutlineView() {
}

@property (strong) NSMapTable *gradientsLineColors;
...
- (NSColor *)lineColorForGradient:(NSGradient *)pGradient location:(CGFloat)pLocation {
	...
	if(color == nil) {
		...
		color = [[NSColor colorWithRGB:(0x000000 + (0x121212 * pLocation)) alpha:1.0] blendedColorWithFraction:0.9
																									   ofColor:color];
...
- (void)collapseItem:(id)pItem collapseChildren:(BOOL)pCollapseChildren {
	[super collapseItem:pItem collapseChildren:pCollapseChildren];
	[self setNeedsDisplay:YES];
}
- (void)expandItem:(id)pItem expandChildren:(BOOL)pExpandChildren {
	[super expandItem:pItem expandChildren:pExpandChildren];
	[self setNeedsDisplay:YES];
}
...
- (void)highlightSelectionInClipRect:(NSRect)pClipRect {
	...
	void (^drawRange)(NSRange range) = ^(NSRange range){
		...
		r.origin.x -= 1.0;
		r.size.width += 2.0;
		[self drawHighlightGradientInRect:r strokeEdges:(ZWEdgeMinY | ZWEdgeMaxY)];
	};
...
- (void)drawHighlightGradientInRect:(NSRect)pRect strokeEdges:(ZWEdges)pStrokeEdges {
	...
	if(pStrokeEdges & ZWEdgeMinX) {
		NSRectFillEdge(pRect, NSMinXEdge, 1.0);
	}
	if(pStrokeEdges & ZWEdgeMaxX) {
		NSRectFillEdge(pRect, NSMaxXEdge, 1.0);
	}
	if(pStrokeEdges & ZWEdgeMinY) {
		NSRectFillEdge(pRect, NSMinYEdge, 1.0);
	}
	if(pStrokeEdges & ZWEdgeMaxY) {
		[[self lineColorForGradient:self.highlightGradient location:1.0] set];
		NSRectFillEdge(pRect, NSMaxYEdge, 1.0);
	}
}
